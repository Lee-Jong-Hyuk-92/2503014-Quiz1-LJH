...
def main():
    ...
    ax[0, 0].scatter(data_x, data_y, c=colors, s=2, alpha=0.7)
    ...
    for i in x_ax:
        ...
        ax[1, 0].bar(x_ax, data1[i], bottom=np.sum(data1[:i], axis=0))
        
    # figure의 (1,0) 위치에서 x축 데이터를 병렬적으로 설정합니다.
    ax[1, 0].set_xticks(x_ax)
    # figure의 (1,0) 위치에서 x축 label을 'A', 'B', 'C'로 설정합니다.
    ax[1, 0].set_xticklabels(['A', 'B', 'C'])
